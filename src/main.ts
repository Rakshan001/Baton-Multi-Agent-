 ...
 *
 * Not the bin entry: `baton` points at cli.ts, which checks the Node floor and
 * then imports this file. Everything below is therefore free to assume a
 * supported runtime — and to pull in the whole command tree while doing it.
 */
...
program
  ...
  // Read from package.json, never hardcoded: a published binary that
  // misreports its own version makes every bug report ambiguous, and the
  // hardcoded copy had already drifted from the field it was meant to mirror.
...
program
  ...
  .option('--serve', 'deprecated — the dashboard is always available (flag ignored)')
  .option('--headless', 'deprecated — agents always get MCP access (flag ignored)')
