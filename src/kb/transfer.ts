...
/** First line of the README below — the proof that a `kb/` is ours to replace. */
const SHARE_README_HEADER = '# Shared knowledge base (generated by `baton kb`)';

/**
 * Is this `kb/` one we generated, and therefore safe to delete and rewrite?
 *
 * `kb/` is an ordinary directory name — a knowledge-base service, a `kb/`
 * package, a folder of notes. Share mode used to `rm -rf` it sight unseen, and
 * the y/N prompt that turns share mode on ("Share the knowledge base via git?")
 * never mentions that it deletes `./kb`. Worse, every later `baton kb rebuild`
 * did it again, so anything a teammate added under the directory the generated
 * README tells them to commit vanished on the next rebuild.
 *
 * The README's own first line is the marker, so directories generated by older
 * versions are recognised too. A `kb/` we did not write is refused, not merged:
 * guessing which files inside were ours is how you delete the other ones.
 */
async function isGeneratedShareDir(dir: string): Promise<boolean> {
  try {
    const head = await readFile(join(dir, 'README.md'), 'utf-8');
    return head.startsWith(SHARE_README_HEADER);
  } catch {
    return false;
  }
}

export class ShareDirConflictError extends Error {
  constructor(dir: string) {
    super(
      `refusing to overwrite ${dir} — it already exists and was not generated by baton.\n`
      + '  Share mode replaces that whole directory on every rebuild.\n'
      + '  Move it aside (or turn share mode off with: baton kb share off) and try again.',
    );
    this.name = 'ShareDirConflictError';
  }
}
...
export async function writeShareDir(root: string, state: KbState): Promise<string> {
  ...
  if (existsSync(dir) && !(await isGeneratedShareDir(dir))) throw new ShareDirConflictError(dir);
