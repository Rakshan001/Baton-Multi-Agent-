...
export function renderOrientation(parts: OrientParts, maxChars: number = ORIENT_MAX_CHARS): string {
  ...
  const durable = [codebase, memory, reports].filter(Boolean);
  if (durable.length === 0) {
  ...
  let sections = [codebase, memory, reports].filter(Boolean);
...
export async function buildOrientation(root: string, opts: { topic?: string } = {}): Promise<string> {
  let memorySection = '';
  try {
  ...
  return renderOrientation({ hasCodebaseMd, memorySection, reports });
}
...
export async function orientForCwd(cwd: string = process.cwd(), opts: { topic?: string } = {}): Promise<string> {
  ...
  return buildOrientation(root, opts);
}
