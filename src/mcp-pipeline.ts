...
import { blockers, eligibleFor, isTerminal, phaseOf, reviewableBy, stateOf, takeable } from './pipeline.js';
...
export function registerPipelineTools(reg: RegisterTool, root: string): void {
  reg(
    ...
    async () => {
      ...
      const startable = eligibleFor(who.agent, tasks) as Task[];
  ...
  reg(
    ...
    async (args: ToolArgs) => {
      ...
      let target = slug;
      if (!target) {
        const pick = nextFor(who.agent, tasks);
        if (!pick) {
          return asText({
            claimed: null,
            why: 'Nothing is startable for you right now.',
            waitingOn: blockers(tasks).slice(0, 10),
            tip: 'Call my_tasks — there may be a review waiting for you, which is what lifts the phase barrier.',
          });
        }
        target = pick.slug;
      }
