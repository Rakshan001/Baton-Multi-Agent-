...
name: prompt-master
version: 1.8.0
description: Generates optimized prompts for AI tools. Activates only when the user explicitly asks to write, fix, improve, or adapt a prompt for a specific AI tool (LLM, Cursor, Midjourney, image AI, video AI, coding agents, etc.). Does not activate for general conversation, coding tasks, document writing, or other non-prompt-engineering work. Use whenever the user says "/prompt-master", "use the prompt master skill", "write me a prompt", "improve this prompt", or pastes a prompt to fix, shorten, split, or adapt for another tool.
---
...
## With Baton (optional — skip cleanly if absent)

Baton's shared memory is the same thing as the **Memory Block** above, except it survives the
session. Use it to spend fewer of your 3 clarifying questions and to stop re-asking settled
questions across sessions.

- **Before asking anything, recall.** `recall_memory` (Baton MCP) or `baton memory list` often
  already answers the *Target tool*, *Context*, and *Constraints* dimensions — which model this
  repo targets, which stack is locked, what was tried and failed. A question you can answer from
  memory is a question you do not spend.
- **Fill the Memory Block from recall, not from the user.** Facts come back anchored to commits and
  file hashes; a fact whose anchor has changed is *withheld* rather than served, so what you paste
  into the prompt is current by construction. Never carry forward a withheld fact.
- **Save the routing decision once.** When the target tool or model is settled for this repo, write
  it with `save_memory` (or `baton memory add`) — one line, e.g. "prompts for this repo target
  Claude Code on Opus 5; Cursor is not used." The next session inherits it.
- **Prompts aimed at agents that will edit *this* repo** get real anchors: pull the file and symbol
  names from `CODEBASE.md` (or the knowledge graph) instead of writing a global instruction, call
  `check_files` first so the scope lock does not collide with a session already editing those
  paths, and tell the operator to run the agent inside an isolated worktree (`baton new "<task>"`).
  This is the Scope-failure row of the Diagnostic Checklist, with the paths actually verified.

None of this is required. With no Baton daemon running, fall back to asking the user and continue
...
*Vendored from **prompt-master** (github.com/nidhinjs/prompt-master), upstream version 1.8.0.
Copyright (c) 2026 Nidhin Joseph Nelson — MIT License; the full permission notice ships as this
skill's `NOTICE`. The "With Baton" section above is a Baton addition under the same MIT terms.*
