...
| **Site hosting readiness + dashboard edge cases** | env-driven site URL (`NEXT_PUBLIC_SITE_URL`), PNG OG image + favicon, correct quick-start commands, mobile nav menu; SSE reconnect indicator, honest error/loading/empty states on Memory/Activity/Conflicts/Knowledge Graph pages, overflow fix | `cd site && npm run build` → `/opengraph-image` + `/apple-icon` routes listed; dashboard → Memory/Activity/Conflicts/Knowledge Graph with demo OFF |
