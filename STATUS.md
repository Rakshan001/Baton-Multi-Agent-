...
| **Skills (catalog + install)** | Searchable catalog of reusable agent playbooks: bundled set in `src/skills/catalog.ts` (incl. a `common-bug-fix` skill that maps the repo → confirms root cause to 95% → fixes bugs one at a time in isolated worktrees), plus imported skills read from `.baton/skills/*.md`. `GET /api/skills` returns each skill with per-agent install state; `POST/DELETE /api/skills/:id/install` writes/removes the skill in the agent's own format — Claude → `.claude/skills/<id>/SKILL.md`, Cursor → `.cursor/rules/<id>.mdc` (`alwaysApply:false`); other CLIs surfaced as unsupported. `POST /api/skills/import` adds a skill from a path or http(s) URL (frontmatter-parsed, 256KB cap, can't shadow a bundled id). All writes gated on `--write`. Dashboard **Skills** screen: search, source/produces chips, per-agent install toggles, playbook preview, import; demo mode mirrors the catalog (`web/src/lib/demoSkills.ts`). | dashboard → Skills; `curl -XPOST localhost:7077/api/skills/common-bug-fix/install -d '{"agent":"claude"}'` writes `.claude/skills/common-bug-fix/SKILL.md` |
...
Tests: 194 vitest tests at root green (routing v2 + MCP-connect + roster + skills covered;
`test/skills.test.ts` covers the render/parse/target helpers). Both workspaces strict TS,
...
src/skills/           skill catalog (bundled) + install/import (.claude/skills, .cursor/rules)
