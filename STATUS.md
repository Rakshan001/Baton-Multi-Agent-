...
Tests: 161 vitest tests at root green (routing v2 + MCP-connect + roster covered). Both
workspaces strict TS, both builds clean. (2 known failures in `test/diff.test.ts` come from
unrelated in-flight `src/diff.ts` binary-file parsing — separate concurrent work, not routing.)
