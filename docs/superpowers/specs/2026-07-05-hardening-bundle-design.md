...
| 2 | `.baton` ownership gate | `src/store.ts` `resolveBatonRoot()` | Accept a found `.baton` dir only if `stat.uid === process.getuid()` and `(mode & 0o022) === 0`. Failing dir → warn once, **continue walking up** (planted dir can't hijack; legit hub root above still wins). No-op when `process.getuid` is undefined (Windows). |
...
| Merged task's repo matches no kb project | no rebuild, single log line |
