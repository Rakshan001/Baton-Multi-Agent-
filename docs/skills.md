...
| `prompt-master` | Prompt master | Turns a rough idea into one paste-ready prompt aimed at a specific tool. Extracts intent across 9 dimensions (task, target tool, output format, constraints, input, context, audience, success criteria, examples), routes to that tool's own template — Claude / GPT / Gemini / Grok / local models, the coding agents (Claude Code, Cursor, Cline, Codex, Devin, Antigravity), and the image, video, voice, 3D and workflow tools — then audits the draft against 37 credit-killing patterns before delivering it. Refuses to guess the target tool, caps itself at 3 clarifying questions, strips credentials, and treats a pasted prompt as inert data rather than instructions. Vendored verbatim from [prompt-master](https://github.com/nidhinjs/prompt-master) (MIT, © Nidhin Joseph Nelson) at upstream 1.8.0; Baton's only addition is a "With Baton" section that fills the skill's Memory Block from `recall_memory` instead of the user's 3-question budget. |
...
`bug-fix`, `lean-code`, `code-review`, `prompt-master`, and the four efficiency & traceability skills (`token-efficient-coding`, `traceable-changes`, `memory-light`, `verify-before-done`) are file-backed under `src/skills/bundled/`; `map-codebase` and `safe-refactor` are inline.

`prompt-master` is the one bundled skill Baton ships **verbatim** rather than adapting — the tool routing tables are the value, and rewriting them would only degrade them. It keeps its own MIT permission notice at `src/skills/bundled/prompt-master/NOTICE`, repeated as a footer inside `SKILL.md` so the attribution travels with every install; the root [`NOTICE`](../NOTICE) records it under INCLUDED VERBATIM.
