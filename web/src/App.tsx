...
import type { Meta, AgentId } from "./types";
...
function ProjectSwitcher({ project, onProject }: { project: Project; onProject: (id: string) => void }) {
  ...
  return (
    <div style={{ position: "relative" }} ref={ref}>
      <button className="fr" onClick={() => setOpen((o) => !o)} aria-haspopup="true" aria-expanded={open} data-tip={project.path} data-tip-side="bottom" style={{
        ...
        <span style={{ width: 14, height: 14, borderRadius: 4, background: project.color, flex: "none", display: "grid", placeItems: "center", color: "#fff", fontSize: 9, fontWeight: 800 }}>{project.name[0]}</span>
      ...
      {open && (
        <div role="menu" style={{ position: "absolute", top: 38, left: 0, width: 300, background: "var(--bg-elevated)", border: "1px solid var(--border-strong)",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 7, padding: "6px 9px 8px" }}>
            ...
            <span className="mono" style={{ fontSize: 11, color: "var(--text-tertiary)" }}>{ws.folder}</span>
            <span className="tag" style={{ marginLeft: "auto" }}>{ws.projects.length} projects</span>
          </div>
          {ws.projects.map((p) => {
            const on = p.id === project.id;
            return (
              <button key={p.id} role="menuitem" className="fr" onClick={() => { onProject(p.id); setOpen(false); }} style={{
                ...
                <span style={{ width: 22, height: 22, borderRadius: 6, background: p.color, flex: "none", display: "grid", placeItems: "center", color: "#fff", fontSize: 11, fontWeight: 800 }}>{p.name[0]}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    {p.primary && <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: "var(--ls-caps)", textTransform: "uppercase", color: "var(--clean-text)", background: "var(--clean-soft)", border: "1px solid var(--clean-border)", borderRadius: 99, padding: "1px 5px" }}>live</span>}
                  </div>
                  <div className="mono" style={{ fontSize: 10.5, color: "var(--text-tertiary)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.framework}</div>
                </div>
...
function TopBar({ counts, apiState, lastUpdated, onRefresh, onMenu, onSearch, onLaunch, navigate, prefs, route, project, onProject, demo }: {
  ...
  return (
    <header style={{ height: 54, flex: "none", display: "flex", alignItems: "center", gap: 10, padding: "0 12px 0 14px", borderBottom: "1px solid var(--border-subtle)", background: "var(--bg-surface)", zIndex: "var(--z-sticky)" as unknown as number }}>
      ...
      {!isMobile && <ProjectSwitcher project={project} onProject={onProject} />}
...
export default function App() {
  ...
  const onProject = (id: string) => {
    ...
    setProjectId(id);
    setSelected(null); setFilter(null); setDiffSlug(null); setHandoffSlug(null); setLiveSlug(null); setLaunchOpen(null);
  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", minHeight: 0 }}>
      ...
      {liveSlug && <LiveSession slug={liveSlug} session={selectedRow(liveSlug)} sessions={sessions} onClose={() => setLiveSlug(null)} setSlug={setLiveSlug} onOpenDiff={(s) => { setLiveSlug(null); setDiffSlug(s); }} />}
